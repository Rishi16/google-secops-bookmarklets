// ╔══════════════════════════════════════════════════════════╗
// ║  Live Unassigned Cases – Bookmarklet                    ║
// ║  Version : 1.5.0                                        ║
// ║  Released: 2026-04-10                                   ║
// ╚══════════════════════════════════════════════════════════╝
javascript: (function () {
  ...
  // ─── TOGGLE OFF ───────────────────────────────────────────────────────────
  if (window.__auto) {
    clearInterval(window.__autoInt);
    window.__auto = false;
    var b = document.getElementById("__autoBadge");
    if (b) {
      b.innerText = "Refresh OFF";
      b.style.background = "#e8710a";
      b.style.color = "white";
    }
    var c = document.getElementById("__autoContainer");
    if (c) c.remove();
    return;
  }
  ...
  // ─── MUTUAL EXCLUSION ─────────────────────────────────────────────────────
  if (window.__autoSIR) {
    showToast("⚠️ Live SIR Cases is running — please stop it first before starting Live Unassigned Cases.");
    return;
  }
  ...
  var old = document.getElementById("__autoContainer");
  if (old) old.remove();

  var badge = document.getElementById("__autoBadge");
  if (!badge) {
    var hdr = document.querySelector(".search-result--header");

    var container = document.createElement("div");
    container.id = "__autoContainer";
    ...
    label.innerText = "Live Unassigned Cases";

    badge = document.createElement("div");
    badge.id = "__autoBadge";
  ...
  badge.style.color = "white";
  window.__auto = true;
  document.title = "Live Unassigned Cases";
  ...
  function start() {
    ...
    window.__autoInt = setInterval(function () {
      if (isSelected()) {
  ...
  var wait = setInterval(function () {
    ...
    if (document.querySelector("label.smp-checkbox") || initTries > 20) {
      ...
      clearFilters(function () {
        ...
        setTimeout(function () {
          check("Status", "Open");
          check("Case Stages", "Triage");
          checkAll("Environments", "Default Environment"); // select all except Default
          searchUser("@L1");
